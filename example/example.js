import { types, parseFile, fileToByteArray, parseByteArray, scanUntil } from '../src/index.js';
...
document.querySelector( "#fileInput" ).addEventListener( "input", async event => {
    ...
    if ( file.type === "image/jpeg" ) {
        ...
        ({ data, error, byteArray } = await parseByteArray( byteArray, wavHeader ));
        resultArea.innerText = error ? "An error has occurred during file parsing" : formatJson( data );
...
async function attemptWavCorrection( byteArray, headerData ) {
    ...
    let offset;
    ({ offset, byteArray } = await scanUntil( byteArray, "fmt " ));
    if ( offset === Infinity ) {
    ...
    const secondAttempt = await parseByteArray( byteArray, halfWavHeader, offset );
...
async function findWavAudioBlock( byteArray, headerData ) {
    let offset;
    ({ offset, byteArray } = await scanUntil( byteArray, headerData.dataChunkId, 0 ));
    if ( offset === Infinity ) {
        ...
        // const parsedData = await parseByteArray( byteArray, { audio: dataType }, offset + 8 );
